# Based on Figure 3 of Leclerc et al.
...
event_dat <- data.frame(
  ...
  value = c(2, 2, 1e9),             # Concentration/Amount to add
...
all_results$abx = factor(all_results$abx, levels = levels(all_results$abx)[c(2,3,1,4)])
...
pa = ggplot(all_results) +
  ...
  facet_grid(pha ~ abx) +
  coord_cartesian(ylim = c(0.001, 10e11)) +
  scale_x_continuous(breaks=seq(0,max(results$time),4))+
  theme_bw() +
  labs(y = "CFU/mL or PFU/mL", x = "Time (hours)", colour = "Organism:") +
...
pb = ggplot(all_results2) +
  ...
  scale_x_continuous(breaks=seq(0,max(results$time),4))+
  theme_bw() +
  labs(y = "CFU/mL or PFU/mL", x = "Time (hours)", colour = "Antibiotic:") +
...
ggsave(here::here("Figures", "ef_fig2.png"), width = 10, height = 11)
