...
params <- c(
  #Bacterial Growth (mu in hour^-1)
  mu_v = 0.924,     # Growth rate of Vancomycin-resistant strain
  mu_a = 0.924,     # Growth rate of Ampicillin-resistant strain
  mu_va = 0.856,    # Growth rate of double-resistant strain
  Nmax = 1e9,       # Carrying capacity (max cells/mL)
  
  #Phage Kinetics
  beta = 3e-8,      # Adsorption rate (mL/hour)
  L = 100,          # Burst size (PFU/cell)
  gamma = 0.005,    # Phage decay rate (min^-1)
  alpha = 0.001,    # Probability of transduction
  tau = 0.333,      # Latent period (hours)
  
  #Antibiotic Killing: Vancomycin
  van_kill_max_BV = 0.2, EC_van_BV = 5.0, pow_van_BV = 2.0,     # BV strain
  van_kill_max_BA = 0.5, EC_van_BA = 2.0, pow_van_BA = 2.0,     # BA strain (lower effect)
  van_kill_max_BVA = 0.1, EC_van_BVA = 10.0, pow_van_BVA = 2.0, # BVA (resistant)
  
  #Antibiotic Killing: Ampicillin
  amp_kill_max_BV = 0.5, EC_amp_BV = 2.0, pow_amp_BV = 2.0,     # BV strain
  amp_kill_max_BA = 0.2, EC_amp_BA = 5.0, pow_amp_BA = 2.0,     # BA strain
...
event_dat <- data.frame(
  ...
  value = c(1e9, 5, 5),         # Concentration/Amount to add
  method = c("add", "add", "add") # "add" increases, "replace" overwrites
)

#define initial states 
init.state <- c(Bv = 1e6, Ba = 1e6, Bva = 1e6, 
                Pl = 0, Pv = 0, Pa = 0, 
                amp = 0, van = 0)
...
ggplot(results, aes(x = time)) +
  ...
  geom_line(aes(y = Bva, color = "Double-Resistant (Bva)"), size = 0.8) +
...
ggsave(here::here("Figures", "ef_fig1.png"), height = 10, width = 10)